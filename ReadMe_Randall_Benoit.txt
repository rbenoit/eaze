...
3) API: The Get endpoint which returns a list would quickly return far too much data. So I added severeal filter parameters:
 - Type: 'WebScrape', 'None'
...
4) Testing: I am a big fan of automated testing, and while I don't always follow test driven development to the letter I believe this solution shows the type of testing I'd be looking at in a real world application. There are gaps, notably around the service which was the last piece to be completed. 
...
5) Service: Most places I've worked with have rolled their own task schedular / job schedular service from scratch. I hadn't used Quartz before but was able to make it work - it's definitely something I'll play around with at home more.  There is no testing of the service in the test project due to time constraints.
...
6) Database: The database project included should work - point to any sql server instance or local express, and publish. The web configs are set up for the following databases:
...
That's about it, I've been running the solution